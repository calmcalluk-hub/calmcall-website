...
function sessionPath(id) {
  return `twilio-sessions/${id}.json`;
}

export async function saveSession(id, session) {
  const blob = await put(sessionPath(id), Buffer.from(encryptSession(session)), {
    access: 'public',
...
export function addHistory(session, role, text) {
  session.history = [...(session.history || []), { role, text: cleanText(text, 900), at: new Date().toISOString() }].slice(-12);
}
...
Your job on this call is to have a genuinely natural conversation, understand why the caller is calling, gather useful lead information without interrogating them, and arrange an appropriate next step. You are not a rigid questionnaire.
...
Return ONLY valid JSON matching the supplied schema. The reply must sound like something Darren would actually say aloud. Keep it under 45 words unless a safety or clarification reason genuinely requires more.`;
  ...
  const response = await withTimeout((signal) => fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'You are a production voice-agent decision engine. Output only JSON.' },
          ...
          schema: {
            ...
            properties: {
              ...
              lead: {
                ...
                additionalProperties: false,
                properties: {
                  name: { type: 'string' }, company: { type: 'string' }, industry: { type: 'string' },
                  location: { type: 'string' }, teamSize: { type: 'string' }, reason: { type: 'string' },
                  missedCalls: { type: 'string' }, jobValue: { type: 'string' }, urgency: { type: 'string' },
                  decisionMaker: { type: 'boolean' }, callbackTime: { type: 'string' }, email: { type: 'string' },
                  interest: { type: 'string' }, notes: { type: 'string' }, score: { type: 'integer', minimum: 0, maximum: 100 },
...
export async function synthesizeSpeech(text) {
  ...
  const response = await withTimeout((signal) => fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}?output_format=mp3_22050_32`, {
    ...
    body: JSON.stringify({
      ...
      voice_settings: { stability: 0.42, similarity_boost: 0.82, style: 0.15, use_speaker_boost: true },
    }),
  }), 7000);
...
export async function createDynamicAudio(text) {
  const hash = crypto.createHash('sha256').update(text).digest('hex').slice(0, 32);
  return audioUrlFor(text, `darren-${hash}`);
}

export function twimlForTurn({ text, audioUrl, actionUrl, hangup = false }) {
...
export function initialSession(callSid, from, to) {
  return {
    version: 2, callSid, from, to, createdAt: new Date().toISOString(), turns: 0,
    history: [],
    lead: { name:'', company:'', industry:'', location:'', teamSize:'', reason:'', missedCalls:'', jobValue:'', urgency:'', decisionMaker:false, callbackTime:'', email:'', interest:'', notes:'', score:30, temperature:'cold' },
  };
}

export async function emitLead(session, reason = 'turn') {
  const payload = {
    event: 'voice_lead_update', reason, timestamp: new Date().toISOString(),
    callSid: session.callSid, from: session.from, to: session.to, createdAt: session.createdAt,
    turns: session.turns, lead: session.lead, history: session.history,
  };
  const webhook = process.env.CALMCALL_CRM_WEBHOOK_URL;
  if (webhook) {
    try {
      await fetch(webhook, { method:'POST', headers:{'Content-Type':'application/json','x-calmcall-event':'voice_lead_update'}, body:JSON.stringify(payload) });
    } catch (err) { console.error('CRM webhook failed:', err); }
  }
  try {
    const path = `twilio-leads/${session.callSid || crypto.randomUUID()}.json`;
    await put(path, Buffer.from(encryptSession(payload)), { access:'public', contentType:'text/plain', allowOverwrite:true });
  } catch (err) { console.error('Lead archive failed:', err); }
}
...
export async function sendSms(to, body) {
  ...
  if (!response.ok) throw new Error(`Twilio SMS ${response.status}`);
  return true;
}
